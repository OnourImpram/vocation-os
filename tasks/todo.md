...
- 2026-07-17: final local `npm run safe:publish-check` passed with 103 test files, 680 tests, 53 schemas, 19 evaluator cases, 23 citation records, a 530 component SBOM, 2 Astro pages, external CLI smoke, and a clean production tarball install.
...
- 2026-07-17: the third protected-branch run passed CodeQL and both JavaScript platform jobs. Rust tests passed. Denied Clippy warnings were corrected, and the Linux-only Tauri `glib` advisory is now governed by a fail-closed Windows target dependency proof instead of an unguarded exception.
